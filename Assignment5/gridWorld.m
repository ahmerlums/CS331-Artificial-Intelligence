...
%
%  There are 26 states in total (including the end state)
...
while 1
    ...
    for i=1:25
        ...
        for succ=1:25
            up=up+T(i,succ,1)*V(succ);
            down=down+T(i,succ,2)*V(succ);
            left=left+T(i,succ,3)*V(succ);
            right=right+T(i,succ,4)*V(succ);
        end
     
        V(i) = R(i) + discount*max([up down left right]);
    end

    check = 1;
        for c = 1:25    
            if(abs(V(c)-Vprev(c))>=0.01)
                check = 0;
                break;
            end
        end
