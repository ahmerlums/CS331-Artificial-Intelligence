...
##############################YOUR CODE BELOW HERE############################################
	...
	while 1:
		...
		if element not in visited:
			...
			b = getCellSuccessors(element,self.grid,self.dimensions,last);
			for i in range(0, len(b)):
				successor = b[i];
				if self.grid[successor[0]][successor[1]] == True:
					path1 = list(path);
					path1.append(successor);
					q.push(path1)
...
def implementBestFirst(source, dest, self, world):
	...
	##############################YOUR CODE BELOW HERE############################################
	heap = []
	temp = Heuristic(source,dest)
	heapq.heappush(heap,temp)
	element = source
	last = source
	while 1:
		element=heapq.heappop(heap).get()
		if not element in path:
			path.append(element)
			if element==dest:
				break
...
def implementAstar(source, dest, self, world):
	...
	path = []; 
	heap = []
	temp = Heuristic1(source,dest,source, [source])
	heapq.heappush(heap,temp)
	visited = [];
	...
	while 1:
		...
		if element.get() not in visited:
			visited.append(element.get())

			if element.get() == dest:
				break
			
			print element.get();
			b = getCellSuccessors(element.get(), self.grid,self.dimensions,last);
			i = 0;
			while i < len(b):
				successor = b[i];
				if self.grid[successor[0]][successor[1]] == True:
					path1 = list(path);
					path1.append(b[i]);
					temp = Heuristic1(b[i],dest,source, path1);
					heapq.heappush(heap,temp)
				i=i+1
	print path
##############################YOUR CODE BELOW HERE############################################
